...
for rate in $requestRates; do 	
    ...
    # save logs
    grep ERROR echo-stack.log | grep _edgeRouter | head -n 1 > edgeRouter-$rate-fault.log
    mv echo-stack.log all-$rate.log
    mv *.log $results
...
for invokeProbability in $invokeProbabilities; do 	
    ...
    # save logs
    grep ERROR echo-stack.log | grep _edgeRouter | head -n 1 > edgeRouter-$invokeProbability-fault.log
    mv echo-stack.log all-$invokeProbability.log
    mv *.log $results
    echo "* Log files stored in ${results}"
